...
x_train = np.load('./data/dacon/comp1/train.npy')
y_train = np.load('./data/dacon/comp1/train.npy')
x_pred = np.load('./data/dacon/comp1/submission.npy')
...
for i in range(4):
    ...
    for thresh in thresholds:
        ...
        for col in select_models:
            ...
            y_pred.append(pred)
        selection.fit(select_x_train, y_train[:,i], **settings)

        y_pred = selection.predict(select_x_test)
        ...
        if mae <= best_mae:
            ...
            best_y_pred = selection.predict(select_x_pred)

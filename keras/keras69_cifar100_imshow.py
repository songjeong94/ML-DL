...
model.add(Conv2D(10, (2,2), # (2,2) = 픽셀을 2 by 2 씩 잘른다.
         input_shape=(32,32,3))) #(가로,세로,명암 1=흑백, 3=칼라)(행, 열 ,채널수) # batch_size, height, width, channels
...
modelpath = './model/{epoch:02d}-{val_loss:.4f}.hdf5' #에포를 2자리정수 로스를 점4자리까지의 
checkpoint = ModelCheckpoint(filepath=modelpath, monitor='val loss', save_best_only = True, mode='auto')

model.compile(loss="categorical_crossentropy", optimizer='adam', metrics=['acc'])
hist = model.fit(x_train,y_train, epochs = 10, validation_split=0.2,callbacks=[early, checkpoint])

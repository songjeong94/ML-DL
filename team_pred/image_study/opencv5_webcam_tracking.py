...
while(True):
    ...
    img_mask = cv.morphologyEx(img_mask, cv.MORPH_OPEN, kernel)#검은구멍 메우기
    ...
    for idx, centroid in enumerate(centroid):
        ...
        #리턴값에서 받은 값들로부터 물체주변의 외각박스 좌표값, 물체중심좌표를 얻는다.

        if area > 50: #노이즈로 인해 검출된 작은물질 제거  크기 50이상만 외각박스 만들기
...
#https://webnautes.tistory.com/1246  이미지로도 가능